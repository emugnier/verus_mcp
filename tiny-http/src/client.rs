...
impl ClientConnection {
    ...
    // VERUS-EXTERNAL: This function performs I/O operations (reading from network stream)
    // which are fundamentally unverifiable. It uses &mut types for I/O that Verus doesn't support.
    #[verifier::external]
        ...
        loop {
            ...
            if byte == b'\n' && prev_byte_was_cr {
                ...
                return AsciiString::from_ascii(buf)
                    .map_err(|_e| IoError::new(ErrorKind::InvalidInput, "Header is not in ASCII"));
            }
