...
// Helper function to parse TE header value and find best matching transfer encoding
// Marked as external because it involves complex sorting and iteration over parsed header values
#[verifier::external]
...
fn choose_transfer_encoding(
    ...
    let mut request_headers_iter = request_headers.iter();
    let user_request = request_headers_iter
        ...
        .and_then(parse_te_header_value);
    ...
    R: Read,
{
    ...
    pub fn new(
        ...
        if let Some(additional_headers) = additional_headers {
            loop {
                match additional_headers.try_recv() {
    ...
    R: Read + Send + 'static,
{
    ...
    // VERUS-EXTERNAL: Returns ResponseBox with Box<dyn Read + Send>, which is a multi-trait dyn object not supported by Verus
    #[verifier::external]
