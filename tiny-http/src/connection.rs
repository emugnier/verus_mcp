...
impl Listener {
    ...
    pub(crate) fn accept(&self) -> std::io::Result<(Connection, Option<SocketAddr>)> {
        match self {
            Self::Tcp(l) => l
                .accept()
                .map(|pair| { let (conn, addr) = pair; (Connection::from(conn), Some(addr)) }),
            #[cfg(unix)]
            Self::Unix(l) => l.accept().map(|pair| { let (conn, _addr) = pair; (Connection::from(conn), None) }),
        }
    }
}
...
// VERUS-EXTERNAL: I/O trait implementations require &mut types which Verus doesn't support
...
impl Connection {
    ...
    // VERUS-EXTERNAL: Uses &mut self for I/O operations
    #[verifier::external_body]
