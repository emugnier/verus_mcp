...
// VERUS-EXTERNAL: Contains trait objects with multiple traits (dyn Read + Send + 'static) which Verus doesn't support
...
// VERUS-EXTERNAL: Creates trait objects (dyn Read + Send) which Verus doesn't support
#[verifier::external_body]
    ...
    W: Write + Send + 'static,
{
    ...
    let transfer_encoding = headers_iter.find(|h: &&Header| h.field.equiv("Transfer-Encoding")).map(|h| h.value.clone());
    ...
    let content_length = if transfer_encoding.is_some() {
        ...
        let mut headers_iter2 = headers.iter();
        headers_iter2.find(|h: &&Header| h.field.equiv("Content-Length")).and_then(|h| FromStr::from_str(h.value.as_str()).ok())
    };

    // true if the client sent a `Expect: 100-continue` header
    let expects_continue = {
        let mut headers_iter3 = headers.iter();
        match headers_iter3.find(|h: &&Header| h.field.equiv("Expect")).map(|h| h.value.as_str())
        {
            None => false,
    ...
    let connection_upgrade = {
        let mut headers_iter4 = headers.iter();
        match headers_iter4.find(|h: &&Header| h.field.equiv("Connection")).map(|h| h.value.as_str())
        {
...
impl Request {
    ...
    // VERUS-EXTERNAL: Takes mut self and returns trait object which Verus doesn't support
    #[verifier::external]
    ...
    #[inline]
    // VERUS-EXTERNAL: Returns &mut dyn Read which Verus doesn't support
    #[verifier::external]
    ...
    #[inline]
    // VERUS-EXTERNAL: Takes mut self and returns trait object
    #[verifier::external]
    ...
    // VERUS-EXTERNAL: Returns trait object
    #[verifier::external]
    ...
    // VERUS-EXTERNAL: Calls extract_writer_impl which returns trait object
    #[verifier::external]
    ...
    pub(crate) fn with_notify_sender(&mut self, sender: Sender<()>) {
...
// VERUS-EXTERNAL: Accesses trait object fields
