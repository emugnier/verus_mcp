...
impl HeaderField {
    ...
    {
        AsciiString::from_ascii(bytes).map(|x| HeaderField(x))
    }
...
impl FromStr for HeaderField {
    ...
    fn from_str(s: &str) -> Result<HeaderField, ()> {
        if s.contains(char::is_whitespace) {
            ...
            AsciiString::from_ascii(s).map(|x| HeaderField(x)).map_err(|_e| ())
        }
    }
}
...
impl FromStr for Method {
    ...
    fn from_str(s: &str) -> Result<Method, ()> {
        Ok(match s {
            ...
            "PATCH" => Method::Patch,
            s => {
                let ascii_string = AsciiString::from_ascii(s).map_err(|_e| ())?;
...
impl From<(u8, u8)> for HTTPVersion {
    fn from(tuple: (u8, u8)) -> HTTPVersion {
        let (major, minor) = tuple;
